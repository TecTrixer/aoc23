...
fn main() {
    ...
    for mut lio in io.line_io() {
        let (_, id) = r!(lio, String, U);
        let mut correct = true;
        let mut min_r = 0;
        let mut min_g = 0;
        let mut min_b = 0;
        for mut show in lio.split(";") {
            let mut sum_r = 0;
            let mut sum_g = 0;
            let mut sum_b = 0;
            for mut pair in show.split(",") {
                let (num, col) = r!(pair, U, String);
                match col.as_str() {
                    "red" => sum_r += num,
                    "green" => sum_g += num,
                    "blue" => sum_b += num,
            ...
            min_r = min_r.max(sum_r);
            min_g = min_g.max(sum_g);
            min_b = min_b.max(sum_b);
            if sum_r > 12 || sum_g > 13 || sum_b > 14 {
                correct = false;
            }
        }
        if correct {
